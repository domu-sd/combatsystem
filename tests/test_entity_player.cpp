...
int main() {
    ...
    std::cout << "Test 2: Player takes damage, stays alive...";
    player.takeDamage(30);
    assert(player.getCurrentHP() == 70);
    assert(player.isAlive() == true);
    std::cout << " PASS\n";

    std::cout << "Test 3: Player takes lethal damage, dies...";
    player.takeDamage(200);
    assert(player.getCurrentHP() == 0);
    assert(player.isAlive() == false);
    std::cout << " PASS\n";

    std::cout << "Test 4: Healing a dead player does nothing...";
    player.heal(50);
    assert(player.getCurrentHP() == 0);
    assert(player.isAlive() == false);
    std::cout << " PASS\n";

    std::cout << "Test 5: Healing a live player restores HP...";
